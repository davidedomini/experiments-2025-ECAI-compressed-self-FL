...
class PSFLQuantization extends AggregateProgram
  ...
  private def actualMetric: (py.Dynamic) => () => Double = (model) => {
    val models = includingSelf.reifyField(nbr(quantization(model)))
  ...
  private def quantization(model: py.Dynamic): py.Dynamic =
    utils.quantization(model)
